...
describe('fns', () => {
  ...
  test('should get top-left position', () => {
    ...
    expect(pos).toEqual('position: fixed; top: 10px; left: 10px');
  });
  test('should be bottom-right position', () => {
    const pos = getToastContainerPosition('bottom-right');
    expect(pos).toEqual('position: fixed; bottom: 10px; right: 10px;');
  });
  test('should get bottom-left position', () => {
    const pos = getToastContainerPosition('bottom-left');
    expect(pos).toEqual('position: fixed; bottom: 10px; left: 10px');
  });
  test('should get bottom-center position', () => {
    const pos = getToastContainerPosition('bottom-center');
    expect(pos).toEqual('position: fixed; bottom: 10px; left: 50%');
  });
